...
const preset = {
    ...
    theme: {
        extend: {
            spacing: Object.fromEntries(Object.entries(spaceVars).map(([key, token]) => [key, toCssVar(token)])),
            borderRadius: Object.fromEntries(Object.entries(radiusVars).map(([key, token]) => [
                key,
                toCssVar(token),
            ])),
            opacity: Object.fromEntries(Object.entries(opacityVars).map(([key, token]) => [
                key,
                toCssVar(token),
            ])),
            colors: {
